...
{
	filesystem->AddSearchPath(CFmtStr("%s/l4d/pak01_dir.vpk", path), "GAME", PATH_ADD_TO_HEAD);
	filesystem->AddSearchPath(CFmtStr("%s/l4d", path), "GAME", PATH_ADD_TO_HEAD);
	g_pVGuiLocalize->AddFile("resource/left4dead_%language%.txt");
}

void AddL4D2(const char* path)
{
	filesystem->AddSearchPath(CFmtStr("%s/l4d2/pak01_dir.vpk", path), "GAME", PATH_ADD_TO_HEAD);
	filesystem->AddSearchPath(CFmtStr("%s/l4d2", path), "GAME", PATH_ADD_TO_HEAD);
...
void AddTF2(const char* path)
{
	...
	filesystem->AddSearchPath(CFmtStr("%s/tf/tf2_sound_vo_%language%_dir.vpk", path), "GAME", PATH_ADD_TO_HEAD);
...
void MountExtraContent()
{
	...
	if (steamapicontext->SteamApps()->BIsAppInstalled(440))
	{
		...
		if (gameinfo->GetBool("tf2content"))
			AddTF2(tf2Path);
	}

	if (steamapicontext->SteamApps()->BIsAppInstalled(500))
	{
		char l4dPath[MAX_PATH];
		steamapicontext->SteamApps()->GetAppInstallDir(550, l4dPath, sizeof(l4dPath));

		if (gameinfo->GetBool("l4dcontent"))
			AddL4D(l4dPath);
	}

	if (steamapicontext->SteamApps()->BIsAppInstalled(550))
	{
		char l4d2Path[MAX_PATH];
		steamapicontext->SteamApps()->GetAppInstallDir(550, l4d2Path, sizeof(l4d2Path));

		if (gameinfo->GetBool("l4d2content"))
			AddL4D2(l4d2Path);
	}
